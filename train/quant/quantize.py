...
from build import build_feature_list, build_fenet, build_inner_codec, build_nn_part1
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--inner_codec_quant_weight",
        type=str,
        help="Path to the inner_codec_quant checkpoint",
    )
...
def main(argv):
    ...
    inner_codec = build_inner_codec(
        args.split_ctx, args.inner_codec_weight, args.device, quant=True
    )
    feature_list = build_feature_list(
        nn_part1, args.split_ctx, args.dataset, args.device
    )

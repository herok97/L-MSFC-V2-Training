...
def parse_args(argv):
    ...
    # ---------------------- Main Arguments ---------------------------------|
    parser.add_argument(
        "--split_ctx", type=str, help="dn53 (tvd), alt1 (hieve), obj, seg"
    )
    ...
    parser.add_argument(
        "--inner_codec_weight", type=str, help="Path to the inner_codec checkpoint"
    )
    ...
    parser.add_argument(
        "--quant", type=str2bool, nargs="?", const=True, default=True, help="quant"
    )
...
def save_features(filename, titles, recon_features, features, mae_list):
    ...
    for i, (title, recon_feature, feature) in enumerate(
        zip(titles, recon_features, features)
    ):
        ax_gt = axes[0, i]
        ax_recon = axes[1, i]
        ...
        im_recon = ax_recon.imshow(
            recon_feature.detach().cpu().numpy(), cmap=plt.cm.jet
        )
...
def encode(args):
    ...
    inner_codec = build_inner_codec(
        args.split_ctx, args.inner_codec_weight, args.enc_device, args.quant
    )
    feature_list = build_feature_list(
        nn_part1, args.split_ctx, args.dataset, args.enc_device
    )
    ...
    for i, features in tqdm(enumerate(feature_list)):
        ...
        inner_codec.encode(
            1, out_fenet["y"], bin_path / f"{i+1}.bin", out_fenet["maxChShape"]
        )
...
def decode(args):
    ...
    inner_codec = build_inner_codec(
        args.split_ctx, args.inner_codec_weight, args.dec_device, args.quant
    )
...
def evaluate(recon_feature_list, args):
    nn_part1 = build_nn_part1(args.split_ctx, args.enc_device)
    feature_list = build_feature_list(
        nn_part1, args.split_ctx, args.dataset, args.dec_device
    )
    mae = AverageMeter()
    titles = {
        "obj": ["p2", "p3", "p4", "p5"],
    ...
    for i, (recon_features, features) in tqdm(
        enumerate(zip(recon_feature_list, feature_list))
    ):
        mae_list = []
